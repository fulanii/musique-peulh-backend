...
class SongEditSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if (
            Song.objects.filter(title=data["new_title"], artist_name=data["new_artist_name"])
            .exclude(id=song_id)  # don't match the song we're editing
            .exists()
        ):
            raise serializers.ValidationError({"detail": f"Artist '{data['new_artist_name']}' already has a song titled '{data['new_title']}'."})
